...
const TodoListTitle: FC<TodoListTitleProps> = () => {
    ...
    const { selectedTag } = useAppSelector((state) => state);
    const { changeSelectedTag } = useActions();
    const [updateTag, {}] = tagAPI.useUpdateTagMutation();

    const handleClick = () => {
        setNewTitle(selectedTag.title);
        if (selectedTag.id !== 0) setEditeble(true);
    };
    ...
    const sumbitHandler: React.FormEventHandler<HTMLFormElement> = async (
        event: React.FormEvent<HTMLFormElement>,
    ) => {
        setEditeble(false);
        updateTag({ ...selectedTag, title: newTitle });
        changeSelectedTag({ ...selectedTag, title: newTitle });
    };
    ...
    return (
        <>
            {editeble ? (
                ...
                <Typography
                    className="todolist__title todolist__title-tag"
                    variant="h1"
                    onClick={handleClick}
                    sx={{ color: selectedTag.color }}>
                    {selectedTag.title}
